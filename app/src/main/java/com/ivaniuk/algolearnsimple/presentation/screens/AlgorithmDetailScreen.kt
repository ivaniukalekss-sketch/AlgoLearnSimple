...
fun AlgorithmDetailScreen(
    ...
    Scaffold(
        ...
        if (algorithm == null) {
            ...
            """.trimIndent()
                            "O(V²) или O(E + V log V) с приоритетной очередью" -> """
    Дейкстра имеет две версии сложности:
    
    • O(V²) - для простой реализации с массивом
    • O(E + V log V) - с приоритетной очередью (кучей)
    
    Где:
    • V - количество вершин
    • E - количество рёбер
    
     Пример с очередью (рекомендуется):
    • V=10, E=15 → ~45 операций
    • V=100, E=300 → ~500 операций  
    • V=1000, E=5000 → ~13,000 операций
    
     Ключевые особенности:
    • Работает только с неотрицательными весами
    • Находит кратчайшие пути от одной вершины до всех остальных
    • Используется в GPS навигаторах и сетевых маршрутизаторах
""".trimIndent()
