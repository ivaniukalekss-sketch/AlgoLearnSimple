...
class GetAlgorithmsUseCaseTest {
    ...
    fun setup() {
        ...
        getAlgorithmsUseCase = GetAlgorithmsUseCase(repository)
    }

    @Test
    fun testInvokeReturnsFlowOfAlgorithmsFromRepository() {
        runBlocking {
            val expectedAlgorithms = listOf(
                Algorithm(1, "Bubble Sort", "", AlgorithmCategory.SORTING, "O(n²)", "", emptyList(), false)
            )
            `when`(repository.getAllAlgorithms()).thenReturn(flowOf(expectedAlgorithms))

            val result = getAlgorithmsUseCase()

            result.collect { algorithms ->
                assertEquals(expectedAlgorithms, algorithms)
            }
            verify(repository).getAllAlgorithms()
        }
    }
}